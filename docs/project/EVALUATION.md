...
`InMemoryHealthTracker` / `DeploymentHealthSnapshot` remain the immediate process-local resilience boundary and are not historical window reconstruction. Cancellation, generator close, unrepresentable post-call failures and recorder failures invalidate sample-source completeness conservatively instead of fabricating provider errors. Shared ingestion, fleet/source-membership completeness and production backend adapters remain pending. Only a subsequent explicit versioned ranking policy may define how operational measurements influence score dimensions.
