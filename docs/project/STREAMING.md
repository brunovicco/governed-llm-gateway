...
The production Gateway application applies a fixed 8 MiB hard ceiling to incoming JSON bodies for
`POST /v1/generate`, `/v1/chat/completions`, `/v1/messages`, and `/v1/responses`. A declared
`Content-Length` above the ceiling is rejected before downstream request
...
capacity statement, ranking input, or authorization policy. Inline media blocks have a stricter
per-block decoded limit. The middleware does not buffer or transform the SSE
response path and does not apply this generation-specific ceiling to unrelated routes.

Tool-result continuation uses explicit prior tool-call state in canonical blocks. Because the
application may already have performed a side effect, that request is single-attempt with no fallback.
