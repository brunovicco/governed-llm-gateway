...
12. Thin client SDK transport — CURRENT; ADR-0010 accepted, implementation/validation in progress.
...
- HTTPS-only base URL; no userinfo/query/fragment; redirects disabled;
...
The first Phase 12 slice also closes an SSE compatibility gap required by the acceptance criterion:
`score_provenance_mode` and `manual_override_id` must be serialized alongside
...
Phase 13 must not begin until the Phase 12 client transport is fully quality-gated, independently
reviewed where required, and merged.
