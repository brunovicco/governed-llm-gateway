...
from governed_llm_gateway_api.route_explain_request_security import (
    MAX_ROUTE_EXPLAIN_REQUEST_BODY_BYTES,
...
def _run(
    app: RouteExplainRequestBodyLimitMiddleware | RouteExplainNoStoreMiddleware,
    scope: Scope,
...
def test_declared_oversize_is_rejected_before_route_explain_parsing() -> None:
    app = RecordingApp()
    middleware = RouteExplainRequestBodyLimitMiddleware(app, max_body_bytes=8)

    sent = _run(middleware, _scope(content_length=9), [_request(b"{}")])

    assert app.calls == 0
    assert _status(sent) == 413
    assert _response_body(sent) == b'{"detail":{"code":"route_explain_request_too_large"}}'
...
def test_non_route_explain_routes_are_not_subject_to_body_limit() -> None:
    app = RecordingApp()
    middleware = RouteExplainRequestBodyLimitMiddleware(app, max_body_bytes=8)

    sent = _run(
        middleware,
        _scope(path="/v1/generate", content_length=999),
        [_request(b"{}")],
    )
...
def test_no_store_wraps_route_explain_body_limit_rejections() -> None:
    app = RecordingApp()
    body_limit = RouteExplainRequestBodyLimitMiddleware(app, max_body_bytes=8)
    middleware = RouteExplainNoStoreMiddleware(body_limit)
...
def test_gateway_composition_installs_route_explain_limit_inside_no_store() -> None:
    ...
    assert middleware_classes.count(RouteExplainRequestBodyLimitMiddleware) == 1
    assert middleware_classes.count(RouteExplainNoStoreMiddleware) == 1
    assert middleware_classes.index(RouteExplainNoStoreMiddleware) < middleware_classes.index(
        RouteExplainRequestBodyLimitMiddleware
    )
    assert MAX_ROUTE_EXPLAIN_REQUEST_BODY_BYTES == 8 * 1024 * 1024
